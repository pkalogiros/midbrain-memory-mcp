...
export async function runSweep(flags) {
  if (flags.required) throw new Error('For release verification use run --xhigh --required, not sweep.');
  const broadProfile = flags.high ? 'high' : flags.xhigh ? 'xhigh' : null;
  ...
  const baseline = broadProfile || quickSimple || flags['model-checks'] ? null : loadBaseline(flags['follow-up']);
  ...
  const jobs = rounds.map(round => ({ resources: [], run: async () => {
    ...
    const profile = broadProfile ? ['--' + broadProfile] : quickSimple ? ['--simple', '--mode', 'registry', ...(flags.config ? ['--config', path.resolve(flags.config)] : [])] : baseline ? ['--follow-up', baseline.dir] : ['--model-checks'];
  ...
  const summary = { profile: broadProfile || (quickSimple ? 'simple' : 'model-checks'), baseline: baseline?.results.run.runId ?? null, infrastructureVerified: Boolean(baseline), totalWorkers: workers, workersPerRound: perRun, parallelRuns, durationMs: Date.now() - started, ok: results.every(r => r.ok), rounds: results, costs: combineCosts(results.map(r => r.costs)) };
  ...
  const coverage = broadProfile ? `${broadProfile === 'high' ? 'High: all scenarios, upgrades and a directed cross-client cycle' : 'XHigh: all scenarios, upgrades and every ordered cross-client pair'} for each round’s selected clients. This is not release sign-off.` : quickSimple ? 'Simple: three prompts per client for capture, fresh-session recall and an unrelated answer. Upgrade and client-specific checks are omitted.' : baseline ? `Infrastructure evidence is reused from verified baseline ${summary.baseline}.` : 'Model checks only: project isolation, upgrade and client-specific behavior are unverified. This is not release sign-off.';
