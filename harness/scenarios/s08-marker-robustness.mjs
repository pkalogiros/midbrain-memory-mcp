...
export default {
  ...
  async run({ ctx, api, client, project }) {
    ...
    const t = seed?.wTurn || await runTurn({ ctx, client, project, prompt, scenarioId: this.id, label: 'turn-1' });
    const rb = seed?.rb || await readback(ctx, api, m, { sinceIso: since, minUser: 1, minAssistant: 1 });
    const readbackPath = t.jsonPath.replace(/\.json$/, seed ? '.literal.readback.json' : '.readback.json');
